...
tools: Grep, Glob, Read, LS, Write, mcp__serena__search_for_pattern, mcp__serena__find_symbol, mcp__serena__get_symbols_overview
model: sonnet
---
...
- `task_description`: What patterns to find
...
{What patterns found, relevance to task}

### Patterns Found ({N} total)
- {pattern1}: {file:line} - {brief description}
...
- Test pattern location: {file:lines}
...
- Suggest this may be a new pattern for the codebase
- Set `status: complete` with `patterns_found: 0`

If too many patterns (>20):
- Focus on most relevant/common ones
- Note total count in summary
- Set `confidence: medium` and explain selection criteria
